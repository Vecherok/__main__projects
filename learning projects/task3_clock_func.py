...
def time_to_words(hr, min):
    ...
    if min == 0:
        if hr == 0 or 5 <= hr <= 11:
            return (f"{text[hr]} часов ровно")
        elif hr == 1:
            return(f"{infin[hr]} час ровно")
        elif hr == 2:
            return(f"{infin[hr]} часа ровно")
        else:
            return(f"{text[hr]} часа ровно")
    elif min == 1 or min == 21 :
        return(f"{text[min]} минута {rodit_sing[hr+1]}")
    elif 2<= min <= 4 or 22 <= min <= 24:
        return(f"{text[min]} минуты {rodit_sing[hr+1]}")
    elif 5 <= min <= 20 or 25 <= min <= 29:
        return(f"{text[min]} минут {rodit_sing[hr+1]}")
    elif min == 30:
        return(f"половина {rodit_sing[hr+1]}")
        ...
        return(f"без {rodit_plur[min]} минуты {rodit_sing[hr+1]}")
    else:
        return(f"без {rodit_plur[min]} минут {rodit_sing[hr+1]}")
